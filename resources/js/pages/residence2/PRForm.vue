<template>
    <div class="w-full flex justify-center">
        ...
        <button type="button" @click="openModal" aria-haspopup="dialog" aria-controls="pr-computations-dialog"
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
        ...
        <!-- Modal Box -->
<div id="pr-computations-dialog" role="dialog" aria-modal="true" aria-labelledby="pr-computations-title"
    class="bg-white w-[95%] sm:w-[90%] md:w-[500px] p-4 sm:p-6 rounded-lg shadow-lg relative max-h-[90vh] overflow-y-auto"
    @click.stop>

            <!-- Close Button -->
            <button type="button" aria-label="Close private residence computation request"
    ...
    &times;
            ...
            <!-- Modal Title -->
            <h2 id="pr-computations-title" class="text-2xl font-semibold text-center mb-4">Request for Computations</h2>

            <!-- Form -->
            <form @submit.prevent="saveInquiry()">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="pr-first-name" class="text-gray-700">First Name*</label>
                        <input id="pr-first-name" v-model="form.first_name" type="text" autocomplete="given-name"
                    ...
                    <div>
                        <label for="pr-last-name" class="text-gray-700">Last Name*</label>
                        <input id="pr-last-name" v-model="form.last_name" type="text" autocomplete="family-name"
                            class="w-full border border-gray-300 rounded-lg p-2 text-gray-900 bg-white"
                ...
                <div class="mt-4">
                    <label for="pr-email" class="text-gray-700">Email*</label>
                    <input id="pr-email" v-model="form.email" type="email" autocomplete="email"
                        class="w-full border border-gray-300 rounded-lg p-2 text-gray-900 bg-white"
                        placeholder="example@gmail.com" required>
                </div>

                <div class="mt-4">
                    <label for="pr-contact" class="text-gray-700">Contact No.*</label>
                    <input
                    id="pr-contact"
                ...
                <div class="mt-4">
                    <label for="pr-unit-type" class="text-gray-700 font-semibold">Unit Type*</label>
                    <select id="pr-unit-type" v-model="form.unit_type"
                ...
                <div class="mt-4">
                    <label for="pr-message" class="text-gray-700">Message*</label>
                    <textarea id="pr-message" v-model="form.message" required
                        class="w-full border border-gray-300 rounded-lg p-2 h-20 text-gray-900 bg-white"
                        placeholder="Enter your message"></textarea>
                </div>

                <div class="mt-4 flex items-start">
                    <input id="pr-privacy-policy" v-model="form.agree_to_privacy" type="checkbox" class="mr-2" required>
                    <label for="pr-privacy-policy" class="text-sm text-gray-600">
